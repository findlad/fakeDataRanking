...
let startDay = "jan 01";
let count = 0;
//figure out and append the phase length for each iteration
phase1Comb.forEach((iteration) => {
  iteration.sort((a, b) => b.length - a.length);
  let phaseStart = startDay;
  let phaseEnd;
  iteration.forEach((bid) => {
    bid.phaseLength = iteration[0].length;
    bid.phaseStart = phaseStart;

    // endDate.setDate(endDate.getDate() - daysToSubtract);

    bid.endDate = new Date(phaseStart) + Number(iteration[0].length * 86400000);
    bid.startDate = bid.endDate - Number(iteration[0].length * 86400000);
    phaseEnd = new Date(phaseStart) + Number(iteration[0].length * 86400000);
  });
  phaseStart = phaseEnd;
  iteration.sort((a, b) => a.workPackage - b.workPackage);
});

console.log(phase1Comb[0]);
...
//     bid.startDate = phaseLength - bid.length;
//     bid.endDate =
